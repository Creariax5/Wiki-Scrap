            ...
            for (int j : listCircle) {
                ...
                if (j != id) {
                    ...
                    if (distanceToNew < 120) {
                        FcalcX = FcalcX + (820 * puissance * distanceX) / distanceToNew;
                        FcalcY = FcalcY + (820 * puissance * distanceY) / distanceToNew;
                    }
                    ...
                    for (String link : cell.getRow()) {
                        if (Objects.equals(link, newCell.getRow()[0])) {
                            FcalcX = FcalcX - (0.1 * puissance * distanceX) / distanceToNew;
                            FcalcY = FcalcY - (0.1 * puissance * distanceY) / distanceToNew;
                            Graphics g = surface.getGraphics();
